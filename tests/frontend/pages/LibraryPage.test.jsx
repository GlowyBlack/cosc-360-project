...
describe("LibraryPage", () => {
  ...
  it("shows an API error when the library request fails", async () => {
    mockUseAuth.mockReturnValue({
      user: {
        id: "507f1f77bcf86cd799439099",
        username: "Reader",
    ...
    global.fetch.mockImplementation((url) => {
      ...
      if (u.includes("/books/me")) {
        return Promise.resolve({
          ok: false,
          json: async () => ({ message: "Books API failed" }),
        });
      }
      if (u.includes("/requests/me")) {
        return Promise.resolve({ ok: true, json: async () => [] });
    ...
    expect(await screen.findByText(/books API failed/i)).toBeInTheDocument();
  });
});
