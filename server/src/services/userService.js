...
const UserService = {
    async getWishlist(userId) {
        ...
        if (!wishlist) throw httpError(404, "User not found");
    ...
    async updateWishlist(userId, bookId) {
        ...
        if (ownerId === String(userId)) {
            throw httpError(400, "You can't add your own book to your wishlist");
        }
    ...
    async getPublicProfile(userId) {
        ...
        if (!user) throw httpError(404, "User not found");
        if (user.isSuspended) throw httpError(404, "User not found");
