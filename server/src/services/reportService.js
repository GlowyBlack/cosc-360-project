...
const ReportService = {
    async createReport(reporterId, { targetType, targetId, reason }) {
        if (!reporterId || !mongoose.isValidObjectId(String(reporterId))) {
            throw httpError(400, "Invalid reporter");
        }

        const type = String(targetType ?? "").trim();
        if (!TARGET_TYPES.has(type)) throw httpError(400, "Invalid target type");
        const tid = String(targetId ?? "").trim();
        if (!mongoose.isValidObjectId(tid)) throw httpError(400, "Invalid target id");
        ...
        if (!exists) throw httpError(404, "Report target not found");
        ...
        if (dup) throw httpError(409, "You already have an open report for this item");
    ...
    async listReportsForAdmin({ status, targetType } = {}) {
        ...
        if (typeFilter && !TARGET_TYPES.has(typeFilter)) {
            throw httpError(400, "Invalid target type");
        }
    ...
    async updateStatusForAdmin(reportId, status) {
        ...
        if (!ADMIN_PATCH_STATUSES.has(next)) {
            throw httpError(400, "Status must be Open, Reviewed, or Dismissed");
        }
